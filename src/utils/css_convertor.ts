...
function processEffectLst(
    ...
    if (effectLst) {
        ...
        if (outerShdw) {
            ...
            if (align === 't')
                offsetY = -Math.abs(offsetY); // Top alignment
            else if (align === 'b')
                offsetY = Math.abs(offsetY); // Bottom alignment
            else if (align === 'l')
                offsetX = -Math.abs(offsetX); // Left alignment
            else if (align === 'r')
                offsetX = Math.abs(offsetX); // Right alignment
            else if (align === 'tl') {
        ...
        const gradientStops =
            gradFill?.gsLst?.gs?.map((stop: any) => {
                const color = stop?.srgbClr?.val
                    ? `#${stop.srgbClr.val}`
                    : '#000000';
                const position = stop?.pos
                    ? `${parseInt(stop.pos) / 1000}%`
                    : '0%';
    ...
    if (style) {
        ...
        for (const [attrib, val] of Object.entries(style)) {
            if (attrib === 'fillRef') {
                ...
                stylecss.color = extractColor(
                    (val as { schemeClr?: any })?.schemeClr,
                );
            }
        }
    }
...
function convertPowerPointStyle(
    ...
    processStyle(
        stylecss,
        node[NodeAttribs.PROPERTIES]?.style,
        maxDim,
        childFrame,
    );
    processProperties(
        stylecss,
        node[NodeAttribs.PROPERTIES],
        maxDim,
        childFrame,
    );
    const newChildFrame = calculateChildFrame(
        node[NodeAttribs.PROPERTIES],
        maxDim,
    );
