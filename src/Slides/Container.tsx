...
const Container: React.FC<ContainerProps> = ({ node, zIndex, mediaPath, maxDim, childFrame }) => {
  ...
  const renderChildren = (node: any, zIndex: number, childFrameParam: any): JSX.Element => {
    ...
    .map((childData: any, index: number) => {
      console.log("renderComponent node:", zIndex, childData._type, childData.asset);
      ...
      } else if (childData._type === "sp" || childData._type === "cxnSp" || childData._type === "grpSp") {
      ...
      } else if (childData._type === "AlternateContent" || childData._type === "Fallback") {
